...
t0<- Sys.time()
da0<- read.table("WAT_M_comb1.txt",sep="\t",header=T,fill=T, quote = "", check.names=F )
tn<- Sys.time()
...
data2$sample<- rep(c("Y","O","CR"), c(6200,4684,4236))
...
# 2.4.3 saveRDS
saveRDS(data2.combined, file = "GSE137869_WAT_Y&O&CR_split_PC1-30_RS0.3_20200721.rds")
...
data2.combined<- readRDS("GSE137869_WAT_Y&O&CR_split_PC1-30_RS0.3_20200721.rds", refhook = NULL)
...
sub_8<- subset(x = data2.combined, idents =17)
...
write.table(zz0,"WAT_cluster17_38_Ncounts_rs0.3_20200721.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
...
dim(zz3)
write.table(zz3,"WAT_cluster17_Y4_Ncounts_rs0.3_20200721.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)


# 2.6 Identify conserved cell type markers
DefaultAssay(data2.combined) <- "RNA"
Conserverd.markers<- FindConservedMarkers(data2.combined, ident.1 = 1, grouping.var = "sample", verbose = FALSE)
head(Conserverd.markers)


# 2.6.1 Running on multiple samples
# https://github.com/hbctraining/scRNA-seq/blob/master/lessons/09_merged_SC_marker_identification.md#running-on-multiple-samples

# 2.6.1.1 Create function to get conserved markers for any given cluster
get_conserved <- function(cluster){
  FindConservedMarkers(data2.combined,
                       ident.1 = cluster,
                       grouping.var = "sample",
                       only.pos = TRUE)%>%
...
library(purrr)
conserved_markers <- map_dfr(c(0,1,2,3,4,5,6), get_conserved)

write.table(conserved_markers,"WAT_cluster0to6_Conserved_Markers_rs0.3_20200721.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)


# 2.6.1.3 Evaluating marker genes
# Extract top 10 markers per cluster???????
top30 <- conserved_markers %>% 
  mutate(avg_fc = ( Y_avg_logFC + O_avg_logFC + CR_avg_logFC)/3) %>% 
  mutate(Gene_symbol = row.names(conserved_markers)) %>%
  group_by(cluster_id) %>% 
...
# 2.7 find markers for every cluster compared to all remaining cells, report only the positive ones
WAT.markers <- FindAllMarkers(data2.combined, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.table(WAT.markers,"WAT_DE markers from clusters_rs0.3_2020722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)

WAT.marker_1<- WAT.markers %>% group_by(cluster) %>% top_n(n = 15, wt = avg_logFC)
write.table(WAT.marker_1,"WAT_DE markers from clusters_top15_rs0.3_20200722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
