...
library(purrr)
zz0<- data2.combined[["RNA"]]@data #normalizes data
table(zz0)
conserved_markers <- map_dfr(c(0,1,2,3,4,5,6,7,8), get_conserved)

write.table(conserved_markers,"BAT_cluster0to8_Conserved_Markers_rs0.3_20200722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
...
BAT.markers <- FindAllMarkers(data2.combined, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.table(BAT.markers,"BAT_DE markers from clusters_rs0.3_2020722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)

BAT.marker_1<- BAT.markers %>% group_by(cluster) %>% top_n(n = 15, wt = avg_logFC)
write.table(BAT.marker_1,"BAT_DE markers from clusters_top15_rs0.3_20200722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
