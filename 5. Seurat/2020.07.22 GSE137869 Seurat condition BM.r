...
library(purrr)
table(Idents(data2.combined)) # check each cluster cell numbers
conserved_markers <- map_dfr(c(0,1,2,3,4,5,6), get_conserved)

write.table(conserved_markers,"BM_cluster0to6_Conserved_Markers_rs0.3_20200722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
...
write.table(BM.markers,"BM_DE markers from clusters_rs0.3_2020722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)

BM.marker_1<- BM.markers %>% group_by(cluster) %>% top_n(n = 15, wt = avg_logFC)
write.table(BM.marker_1,"BM_DE markers from clusters_top15_rs0.3_20200722.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
