...
da0<- readRDS("GSE137869_BAT_Y&O&CR_split_PC1-30_RS0.3_20200721.rds", refhook = NULL)
...
library(SingleR)
Ncounts_singleR<- matrix(Ncounts_seurat, ncol=13135)
dim(Ncounts_singleR)
...
dim(pred)
pred[1:2,]
cbind(0:16,pred$labels ,pred2$labels )
cbind(0:16,pred2$labels) # Exhibit each cluster cell types

pred$scores
pred2$scores # Exhibit possible cell types scores

xx<-cbind(0:16,pred2$labels)
write.table(xx,"SingleR for define celltypes_BAT_rs0.3_20200723.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)

xx1<-pred2$scores
write.table(xx1,"SingleR score for define celltypes_BAT_rs0.3_20200723.txt", sep="\t",row.name=T, col.name=T, quote=FALSE)
